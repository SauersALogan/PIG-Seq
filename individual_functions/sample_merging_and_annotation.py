#!/usr/bin/env python3
"""
annotation_and_normalization.py
==========================================

Function to normalize counts from the feature_counting.py and feature_parsing.py files to RPK, TPM, CPM
Also annotates the locus_tags to interpretable products.
...
def normalize_and_annotate(count_file, gff_file):
    counts = pd.read_csv(count_file, delimiter='\t', header=0)
    count_col = counts.columns[-2]
...
def run_norm_anno(count_files, gff_files):
    ...
    if isinstance(count_files, str) and isinstance(gff_files, str):
        ...
        if not paired_files:
            print(f"DEBUG: No matching file pairs found in your gff and count files, check file naming conventions or documentation")
            return results
        for count_file, gff_file in paired_files:
            normalized_counts = normalize_and_annotate(count_file, gff_file)
            results.append(normalized_counts)
    return(results)
...
@pytest.fixture
def multiple_samples(temp_test_dir):
...
def test_single_assembly(single_sample, single_gff):
    ...
    count_col = result_df.columns[6]
    total_counts = result_df[count_col].sum()
    expected_cpm = (result_df[count_col] / total_counts) * 1_000_000
    print("Function output CPM:", result_df['CPM'].values)
    print("Test expected CPM:", expected_cpm.values)
    print("Difference:", result_df['CPM'].values - expected_cpm.values)
    assert result_df['CPM'].sum() == pytest.approx(1_000_000, rel=1e-3), "CPM doesn't sum to 1M"
    assert result_df['CPM'].equals(expected_cpm), "CPM calculation incorrect"
    expected_rpk = result_df[count_col] / (result_df['Length'] / 1000)
    total_rpk = expected_rpk.sum()
    expected_tpm = (expected_rpk / total_rpk) * 1_000_000
    assert result_df['TPM'].sum() == pytest.approx(1_000_000, rel=1e-3), "TPM doesn't sum to 1M"
    assert 'product' in result_df.columns, "Product column missing"
    assert not result_df['product'].isna().any(), "Missing product annotations"
    prokka_00003 = result_df[result_df['Geneid'] == 'PROKKA_00003']
    assert len(prokka_00003) == 1, "PROKKA_00003 not found"
    assert prokka_00003['product'].iloc[0] == 'hypothetical protein'
    prokka_00004 = result_df[result_df['Geneid'] == 'PROKKA_00004']
    assert prokka_00004['product'].iloc[0] == 'DNA polymerase'
    highest_count_row = result_df.loc[result_df[count_col].idxmax()]
    assert highest_count_row['CPM'] == result_df['CPM'].max(), "Highest count should have highest CPM"
